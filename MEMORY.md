...
- **重命名子代理（v1.3.4 修正，重要）**：descriptor label 不可变；且子代理常**未列入会话列表**，客户端 `binding(childId)` 依赖 `eligible`（在 ids 里），取不到 → 客户端 `binding.session.rename` 静默失败。必须走 **host `renameSubagent` 端点**：`_resolveAgent(sessionId)` 冷恢复拿 live Session → `ctx.sessionTitle.rename(session, name)` 写 durable title；`childName` 优先 `summary.title` 显示。⚠️ **不要用 `summary.displayTitle`**——子代理继承父 cwd，`displayTitle` 是**父工作区名**（用户看到"全部显示父会话名字"），是错误来源。
